...
live = Live('dvclive/flyUTRs', cache_images=True)
...
def copy_data_and_output_to_temp_dir(args, temp_dir=None):
    ...
    #shutil.copytree(args.output_dir, tmp_output, symlinks=True)
...
def load_data(args, tokenizer, test_run=False, split='train.fasta'):
    ...
    for record in data:
        ...
        if len(s) < 10 or len(s) > args.max_seq_length:
            continue
        am = [1]*len(s)
        am += [0]*(args.max_seq_length-len(s))
...
def mask_tokens(inputs, tokenizer, kmer = 4, mask_prob=0.15):
    ...
    return inputs, attention_mask, masked_lm_labels#labels#

def concatTrainTensors(easy, medium, hard=None):
    if hard is None:
        padded_size = medium.size(1) - easy.size(1)
        easy = F.pad(easy, (0, padded_size), "constant", 0)
        return torch.cat([easy, medium], dim=0)
    else:
        padded_size_e = hard.size(1) - easy.size(1)
        padded_size_m = hard.size(1) - medium.size(1)
        easy = F.pad(easy, (0, padded_size_e), "constant", 0)
        medium = F.pad(medium, (0, padded_size_m), "constant", 0)
        return torch.cat([easy, medium, hard], dim=0)
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        if args.curriculumLearning:
            if epoch_counter >= args.numEpochsBeforeEarlyStopping//2:
                train_sampler = RandomSampler(concatTrainTensors(train_dataset[0], train_dataset[1])) if args.local_rank == -1 else DistributedSampler(concatTrainTensors(train_dataset[0], train_dataset[1]))
                train_dataloader = DataLoader(concatTrainTensors(train_dataset[0], train_dataset[1]), sampler=train_sampler, batch_size=args.train_batch_size)
            if epoch_counter >= args.numEpochsBeforeEarlyStopping:
                train_sampler = RandomSampler(concatTrainTensors(train_dataset[0], train_dataset[1], train_dataset[2])) if args.local_rank == -1 else DistributedSampler(concatTrainTensors(train_dataset[0], train_dataset[1], train_dataset[2]))
                train_dataloader = DataLoader(concatTrainTensors(train_dataset[0], train_dataset[1], train_dataset[2]), sampler=train_sampler, batch_size=args.train_batch_size)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # print out memory usage on cuda
            # print(torch.cuda.memory_summary())
            outputs = model(**inputs)
            loss = outputs[0]
            #logger.info("Batch Loss: %f", loss)
            live.log_metric("batch/loss", loss.item())
            #[np.max([b.nonzero().size(0) for b in batch]) for i, batch in enumerate(train_dataloader)]
            if args.n_gpu > 1:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    # model_to_save.model.config.save_pretrained(output_dir)
...
def main():
    ...
    parser.add_argument("--data_dir", default="output/data/flyUTRs", type=str, help="The input data dir. Should contain the .fasta files (or other data files) for the task.",)
    ...
    if args.do_train:
        ...
        val_dataset = load_data(args, tokenizer, args.test_run, 'dev.fasta')
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model_to_save = (model.module if hasattr(model, "module") else model)  # Take care of distributed/parallel training
        # model_to_save.model.config.save_pretrained(args.output_dir)
