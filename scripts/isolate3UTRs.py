...
def parse_args():
    ...
    parser.add_argument('--transcriptSection', type=str,default='three_prime_utr', help='Transcript section to extract. can be multiple sections separated by comma. e.g. three_prime_utr,cds,five_prime_utr')
...
def read_gtf(gtf_file, transcriptSection):
    utr_dict = {}
    with gzip.open(gtf_file, 'rt') as f:
        for line in f:
            if line.strip() and not line.startswith('#'):
                parts = line.split('\t')
                if parts[2] == transcriptSection: #'three_prime_utr':
...
secToName = {'three_prime_utr': '3utr', 'five_prime_utr': '5utr', 'CDS': 'cds'}
def process_files(data_dir, output_dir, files, start_index, transcriptSection):
    global secToName
    os.makedirs(output_dir, exist_ok=True)
    base_name = files[start_index].split('-')[0]
    for sec in transcriptSection.split(','):
        utr_dict = read_gtf(os.path.join(data_dir, files[start_index]),sec)
        utr_sequences = extract_3utr_sequences(os.path.join(data_dir, files[start_index + 1]), utr_dict)
        output_file = os.path.join(output_dir, f'{base_name}-{secToName[sec]}.fa')
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        for i in range(0, len(files) - 1, 2):
            ...
            executor.submit(process_files, args.data_dir, args.output_dir, files, i, args.transcriptSection)
