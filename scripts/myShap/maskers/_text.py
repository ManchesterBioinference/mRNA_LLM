...
class Text(Masker):
    ...
    def token_segments(self, s):
        ...
        try:
            if ',' in s:
                # Split on comma and process each part separately
                utr5, utr3 = s.split(',')
                utr5 = utr5.replace('U', 'T')
                utr3 = utr3.replace('U', 'T')
                
                # Tokenize each part separately
                token_data_5 = self.tokenizer(utr5, return_offsets_mapping=True, add_special_tokens=False)
                token_data_3 = self.tokenizer(utr3, return_offsets_mapping=True, add_special_tokens=False)
                
                # Combine tokens, removing first token from second part
                combined_ids = token_data_5["input_ids"] + [self.tokenizer.sep_token_id] + token_data_3["input_ids"]
                
                # Create a new string that represents the combined parts
                s = self.tokenizer.decode(combined_ids) #need to update s so that the correct parts are returned
                self.s = s
                # Re-tokenize the combined string to get correct offset mappings
                token_data = self.tokenizer(s, return_offsets_mapping=True, add_special_tokens=True)
            else:
                # Original tokenization for strings without comma
